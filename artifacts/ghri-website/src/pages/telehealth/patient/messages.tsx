...
import { MessageSquare, Send, Plus, Search, X, Shield } from "lucide-react";
...
import { useGetMessages, useGetProviders, useSendMessage } from "@workspace/api-client-react";
...
export default function PatientMessages() {
  ...
  const { data: providers } = useGetProviders({
    query: { enabled: !!token, queryKey: ["getProviders", user?.id] },
    ...authHeaders,
  });
  ...
  // Directory: every provider the patient is allowed to message
  const directoryMap = new Map<number, { id: number; name: string; specialty: string | null }>();
  providers?.forEach((p) => directoryMap.set(p.id, { id: p.id, name: p.name, specialty: p.specialty ?? null }));

  // Conversation list = providers you've messaged + any started via "New message"
  const contactMap = new Map<number, { id: number; name: string; specialty: string | null }>();
  const addContact = (id: number) => {
    if (id === user?.id || contactMap.has(id)) return;
    const dir = directoryMap.get(id);
    contactMap.set(id, dir ?? { id, name: `User ${id}`, specialty: null });
  };
  messages?.forEach((m) => addContact(m.senderId === user?.id ? m.recipientId : m.senderId));
  extraContactIds.forEach(addContact);
  const contacts = Array.from(contactMap.values());

  // Providers available to start a NEW conversation (not already in the list)
  const pickerContacts = (providers ?? [])
        ...
        <Card className="flex-1 flex flex-col md:flex-row overflow-hidden shadow-sm border-slate-200 relative">
          ...
          <div className="w-full md:w-64 lg:w-80 border-b md:border-b-0 md:border-r bg-slate-50 flex flex-col">
            <div className="p-4 border-b bg-white flex items-center justify-between">
              ...
              <Button size="sm" className="h-8 gap-1.5 text-xs" onClick={() => setShowPicker(true)}>
            ...
            <div className="flex-1 overflow-y-auto">
              {contacts.length === 0 ? (
                <div className="p-4 text-center text-sm text-slate-500">
                  ...
                  <br />
                  Tap <span className="font-medium text-primary">New</span> to message a provider.
                </div>
              ) : (
                contacts.map((c) => {
                  ...
                  return (
                    <button
                      ...
                      className={cn(
                        ...
                        selectedId === c.id ? "bg-blue-50 border-l-4 border-l-primary" : ""
                      )}
                    >
                      <div className="flex items-center gap-3">
                        ...
                        <div className="overflow-hidden flex-1">
                          <p className="font-semibold text-slate-900 truncate">Dr. {c.name}</p>
                          <p className="text-xs text-slate-500 truncate">
                            {last ? `${last.senderId === user?.id ? "You: " : ""}${last.content}` : c.specialty || "Provider"}
                          </p>
                        </div>
                      </div>
                    </button>
          ...
          <div className="flex-1 flex flex-col bg-white min-h-[400px]">
            {selectedId ? (
              <>
                <div className="p-4 border-b flex items-center gap-3 bg-white">
                  <div className="w-10 h-10 rounded-full bg-slate-100 flex items-center justify-center text-slate-600 font-bold">
                    {selectedContact?.name.charAt(0) || "D"}
                  </div>
                  <div>
                    <h3 className="font-bold text-slate-900">Dr. {selectedContact?.name}</h3>
                        ...
                        <div key={msg.id} className={`flex flex-col ${isMe ? "items-end" : "items-start"}`}>
                          <div
                            className={`max-w-[80%] rounded-2xl px-4 py-2 ${
                              isMe ? "bg-primary text-primary-foreground rounded-tr-sm" : "bg-slate-100 text-slate-900 rounded-tl-sm"
                            }`}
                          >
                ...
                <div className="p-4 border-t bg-slate-50">
                  <form onSubmit={handleSend} className="flex gap-2">
                    <Input
                      ...
                      className="bg-white rounded-full px-4 flex-1"
                      autoComplete="off"
                    />
                    <Button type="submit" size="icon" className="rounded-full shrink-0" disabled={sendMutation.isPending || !content.trim()}>
              ...
              <div className="flex-1 flex flex-col items-center justify-center text-slate-400 gap-4">
                ...
                <Button className="gap-1.5" onClick={() => setShowPicker(true)}>
                ...
                <div className="max-h-80 overflow-y-auto">
                  {pickerContacts.length === 0 ? (
                    <div className="p-6 text-center text-sm text-slate-500">
                      {search ? "No matching providers" : "No providers available"}
                    </div>
                  ) : (
                    pickerContacts.map((p) => (
                      ...
                      >
                        ...
                        <div className="min-w-0 flex-1">
                          <p className="text-sm font-medium text-slate-900 truncate">Dr. {p.name}</p>
